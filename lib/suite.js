...
var setupEventHandler = function(self) {
  return function(benchmark) {
    ...
    for(var i = 0; i < reporters.length; i++) {
      reporters[i].benchmarkSetup(self, benchmark);
    }
...
var teardownEventHandler = function(self) {
  return function(benchmark) {
    ...
    for(var i = 0; i < reporters.length; i++) {
      reporters[i].benchmarkTeardown(self, benchmark);
    }
  }
...
var cycleEventHandler = function(self) {
  return function(cycle, benchmark) {
    ...
    for(var i = 0; i < reporters.length; i++) {
      reporters[i].benchmarkCycle(cycle, self, benchmark);
    }
  }
...
var iterationEventHandler = function(self) {
  return function(iteration, cycle, benchmark) {
    ...
    for(var i = 0; i < reporters.length; i++) {
      reporters[i].benchmarkIteration(iteration, cycle, self, benchmark);
    }
  }
  ...
  execute(context) {
    return new Promise((resolve, reject) => {
      ...
      co(function*() {
        ...
        for(var i = 0; i < reporters.length; i++) {
          reporters[i].suiteTeardown(self);
        }
